...
%C=ST(1,3)
p=ST(1,4)  
r=0.1;   % radius from pumping well (of r of pumping well casing for step test)
makeplots=true;  % makeplots=false;  % False is faster but cannot see progress.

Green =@(r,S,T,tvec) exp(-(r*r*S/4/T./tvec))./tvec;

welldata = importdata('Burg_dd.txt'); % Assumes columns of (t,s)
Q = importdata('Burg_Q.txt');         % Assumes columns of (tstart_i,Q_i)
