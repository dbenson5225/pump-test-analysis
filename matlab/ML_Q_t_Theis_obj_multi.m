...
%Q =sort(Q,1)                         % Make sure times are increasing
...
tmin=0; %  tmaxG=100000*tpeakG; ntpointsG=400000;
tmax=max([max(welldata1(:,1))  max(welldata2(:,1))  max(welldata3(:,1)) max(welldata4(:,1))]);
dt=tpeak/8; % Divisor is points between zero and peak of Green's fn. Check fig 3 for a decent peak!
...
figure(99); loglog(tvec,Qvec,tvec,Green1);
...
%weights(length(weights)-length(obs3)+1:end)=0;  % Eliminate large radius well.
